...
var App = {
    ...
    init: function () {
        ...
        ['one', 'two', 'three'].forEach(function (item, index) {
            html += '<div class="col-md-3 col-sm-3 col-lg-3" style="margin-bottom: 20px;"><div class="number ' + item + '"></div></div>';
        });
    ...
    getImages: function () {
        $.get({
            url: "https://slavagame-server.herokuapp.com/api/images",
            success: function (data) {
                $('#label').html(data[0].src + " " + data[1].src + " " + data[2].src);
            }
        });
